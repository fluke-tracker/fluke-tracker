...
class WhaleUploaderComponent extends React.Component
{
	...
	filesChanged(files)
	{
		this.setState(
		{
			...
			imageNames: Array.from(files).map(
				(item) => item.name
			),
		});
		Promise.all(
	...
	async uploadViaCropping(file, progressCustomCallback)
	{
		...
		var options = {
			...
			progressCallback: (progressEvent) => 
                    progressCustomCallback(50 * progressEvent.loaded / progressEvent.total),
		};
		Storage.put(uploadPath + file.name, file, options)
			.then((result) =>
			{
				this.uploadThumbnail(file, (progressEvent) => {
                    progressCustomCallback(50 + 50 * progressEvent.loaded / progressEvent.total)
                });
	...
	async uploadViaBrowserCropping(file, progressCustomCallback)
	{
		...
		const uploadPath = "cropped_images/";
		console.log(
			this.state.cropperComponents.filter(comp => comp).map(
		...
		Storage.put(uploadPath + croppedFile.name, croppedFile,
			...
			.then((result) =>
			{
				this.uploadThumbnail(croppedFile, 
					(progress) => progressCustomCallback(50 + 50 * progress.loaded / progress.total));
				console.log("image uploaded", result);
				this.upload = null;
				this.addResponse(
					"File " + file.name + " uploaded successfully!",
	...
	{
		return this.state.cropperComponents[i] ? await this.state.cropperComponents[i]?.getCroppedImage() : null
	}
	...
	render()
	{
		...
		const BrowserCropping = this.state.imageFilesStrings.map((image, i) =>
			<CropperComponent filename = {
				this.state.imageNames[i]
			}
			ref = {
				(instance) => this.state.cropperComponents[i] = instance
			}
			workerHandler = {
				this.workerHandler
			}
