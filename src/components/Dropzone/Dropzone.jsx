...
class DropzoneComponent extends React.Component {
  ...
  handleChangeStatus = ({ meta }, status, images) => {
    ...
    var seen = {};
    const files = images
            .filter(image =>seen.hasOwnProperty(image.meta.name) ? false : (seen[image.meta.name] = true))
            .filter(image => image.meta.status !== "removed")
            .map(image => new File([image.file], image.file.name, { type: image.file.type }));
  ...
  setProgress(filename, progress) {
    ...
    this.props.files.filter(file => file.filename === filename).forEach( file => file.progress = progress);
  }
  ...
  handleSubmit = async (files, allFiles) => {
    ...
    console.log("update");
    const dropzone = this.dropzoneRef?.current;
    dropzone.files = await Promise.all(dropzone.files.map(async (f, i) => {
  ...
  render() {
      ...
      const SubmitButton = (props) =>
                ...
                <>
                  ...
                  onClick={() => user ? this.handleSubmit(props.files.filter(f => ['headers_received', 'done'].includes(f.meta.status)), props.files): this.props.history.push(`/login`)}
                  color="success"
                  size="lg"
                >
                  {user ? <><Icon name="cloud upload"/><div>Upload all files</div></> : <><Icon name="address book outline"/><div>Login to upload</div></>}
                </Button>
                </>
        )
      ...
      return (
           <>
            ...
            <Dropzone
                      ...
                      accept="image/*,video/*"
                      inputContent={(files, extra) => (extra.reject ? 'Image and video files only' : 'Drag Whale Images here')}
