...
var RevealChalkboard =
  ...
  (function () {
    ...
    var toggleChalkboardButton =
      config.toggleChalkboardButton == undefined
        ? true
        : config.toggleChalkboardButton;
    var toggleNotesButton =
      config.toggleNotesButton == undefined ? true : config.toggleNotesButton;
    ...
    if (legacyFileSupport) {
      console.warn(
        "Legacy file support is deprecated and may be removed in future versions!"
      );
    }
    ...
    if (toggleChalkboardButton) {
      ...
      button.innerHTML =
        '<a href="#" onclick="RevealChalkboard.toggleChalkboard(); return false;"><i class="fa fa-pencil-square-o"></i></a>';
    ...
    if (toggleNotesButton) {
      ...
      button.innerHTML =
        '<a href="#" onclick="RevealChalkboard.toggleNotesCanvas(); return false;"><i class="fa fa-pencil"></i></a>';
      document.querySelector(".reveal").appendChild(button);
    }
    //alert("Buttons");
    ...
    function createPrintout() {
      ...
      patImg.onload = function () {
        ...
        if (width != storage[1].width || height != storage[1].height) {
          scale = Math.min(
            width / storage[1].width,
            height / storage[1].height
          );
    ...
    function drawWithChalk(context, fromX, fromY, toX, toY) {
      ...
      var opacity =
        Math.min(0.8, Math.max(0, color[1].replace(/^.*,(.+)\)/, "$1") - 0.1)) +
        Math.random() * 0.2;
          ...
          if (
            ...
            if (event.type == "erase") {
              drawingCanvas[mode].sponge.style.left =
                mouseX - eraserDiameter + "px";
              drawingCanvas[mode].sponge.style.top =
                mouseY - eraserDiameter + "px";
            }
          }
